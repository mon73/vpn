...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	echo "Create client With Unlimited connections: "
	echo "   1) Yes"
	echo "   2) No"
	read -p "Type [1]: " TYPE
...
max-clients 50000" > /etc/openvpn/server/server.conf
	# IPv6
	if [[ -z "$ip6" ]]; then
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo "   1) Add a new client [Unlimited Connection]"
